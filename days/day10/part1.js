...
const assign = (state, bot, value) => {
    ...
    if (state.bots[bot].low !== null && state.bots[bot].high !== null) {
        /*
         * THE SOLUTION OF PART 1 IS DETERMINED HERE (MEH)
         */
        if (state.bots[bot].low === 17 && state.bots[bot].high === 61) {
            console.log(bot);
        }
        ...
        if (rule.low.to === BOT) {
            ...
            state.outputs[rule.low.target] = low;
        } else {
        ...
        if (rule.high.to === BOT) {
            ...
            state.outputs[rule.high.target] = high;
        } else {
...
fs.readFile('input.txt').then(data => solve(data.toString()));
